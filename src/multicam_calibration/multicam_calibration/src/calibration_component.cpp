...
void CalibrationComponent::initialize() {
  ...
  // new parameters
  approx_sync_slop_ = declare_parameter("approx_sync_slop", 0.3);
  force_publish_debug_ = declare_parameter("force_publish_debug", false);
  log_raw_frames_ = declare_parameter("log_raw_frames", true);
  single_camera_mode_ = declare_parameter("single_camera_mode", false);
  try {
  ...
  if (single_camera_mode_) {
    RCLCPP_WARN(get_logger(), "single_camera_mode active: only subscribing to first camera stream");
  }
  for (size_t i = 0; i < cameras_.size(); ++i) {
    const auto &camdata = cameras_[i];
    if (single_camera_mode_ && i > 0) { break; }
    // use sensor data qos for camera streams
    rclcpp::SensorDataQoS sensor_qos;
    if (!single_camera_mode_ && cameras_.size() > 1) {
      sub_.push_back(std::make_shared<message_filters::Subscriber<ImageMsg>>(
          this, camdata.rostopic, sensor_qos.get_rmw_qos_profile()));
      RCLCPP_INFO(get_logger(), "created message_filters subscriber for %s with SensorDataQoS", camdata.rostopic.c_str());
    } else {
    ...
    imagePub_.push_back(image_transport::create_publisher(
        this, camdata.name + "/debug_image", sensor_qos.get_rmw_qos_profile()));
    tagCountPub_.push_back(create_publisher<std_msgs::msg::UInt32>(
        camdata.name + "/tag_count", rclcpp::QoS(10)));
    if (log_raw_frames_) {
      auto sub_raw = create_subscription<ImageMsg>(
          camdata.rostopic, rclcpp::SensorDataQoS(),
          [this, topic=camdata.rostopic](const ImageMsg::SharedPtr msg){
            static int cnt = 0; if (cnt < 20) {
              RCLCPP_INFO(this->get_logger(), "RAW frame topic=%s stamp=%u.%u enc=%s", topic.c_str(), msg->header.stamp.sec, msg->header.stamp.nanosec, msg->encoding.c_str());
              cnt++;
            }
            // detect large timestamp offset between first two cameras
            static bool have_first=false, have_second=false, warned=false; 
            static rclcpp::Time t_first, t_second; 
            if (!have_first) { t_first = rclcpp::Time(msg->header.stamp); have_first = true; }
            else if (!have_second && topic != cameras_[0].rostopic) { t_second = rclcpp::Time(msg->header.stamp); have_second = true; }
            if (have_first && have_second && !warned) {
              auto dt = (t_first > t_second ? (t_first - t_second).seconds() : (t_second - t_first).seconds());
              if (dt > 2.0) {
                RCLCPP_ERROR(this->get_logger(), "Header timestamp offset between first two cameras is %.3f s (>2s). message_filters will never sync. Fix driver timestamps or enable single_camera_mode.");
                warned = true;
              }
            }
          });
      raw_log_subs_.push_back(sub_raw);
    }
  }
  // if single camera mode, no need to set up sync
  if (!single_camera_mode_) {
    subscribe();
  }
  //
...
static bool compute_RPQ(unsigned int cam1_idx, unsigned int cam2_idx,
  ...
  if (distm == "equidistant") {
    ...
    cv::stereoRectify(K1, ci1.distortion_coeffs, K2, ci2.distortion_coeffs,
                      cv::Size(ci1.resolution[0], ci2.resolution[1]), R, T, *R1,
                      *R2, *P1, *P2, *Q);
  } else {
...
void CalibrationComponent::subscribe() {
  RCLCPP_INFO(get_logger(), "setting up subscriptions, approx_sync_slop=%.3f", approx_sync_slop_);
  switch (sub_.size()) {
    case 0:
      break;
    case 2:
      if (use_approximate_sync_) {
        ...
        // apply slop (ROS2 message_filters lacks direct API in some distros; try dynamic_cast)
        // fallback: setMaxIntervalDuration exists in humble/rolling
#if defined(RCLCPP_VERSION_MAJOR)
        approx_sync2_->setMaxIntervalDuration(
            rclcpp::Duration::from_seconds(approx_sync_slop_));
#endif
        approx_sync2_->registerCallback(&CalibrationComponent::callback2, this);
        RCLCPP_INFO(get_logger(), "Approx sync2 created queue=60 slop=%.3f", approx_sync_slop_);
      } else {
...
void CalibrationComponent::process(const std::vector<ImageConstPtr> &msg_vec) {
  static int info_count_process = 0;
  if (info_count_process < 5) {
    RCLCPP_INFO(get_logger(), "process() entry frameNum=%d msgs=%zu", frameNum_, msg_vec.size());
    info_count_process++;
  }
  RCLCPP_DEBUG(get_logger(), "process() called with %zu images", msg_vec.size());
...
void CalibrationComponent::processSingle(ImageConstPtr const &img0) {
  // minimal pipeline for single camera debug
  std::vector<ImageConstPtr> vec = {img0};
  CamWorldPoints wp; CamImagePoints ip;
  auto tagsVec = detector_->process(vec, &wp, &ip);
  publishDebugImages(vec, tagsVec);
}
...
void CalibrationComponent::publishDebugImages(
  ...
  for (int i = 0; i < (int)detected_tags.size(); i++) {
    RCLCPP_DEBUG(get_logger(), "publishDebugImages cam %d tags=%zu subs=%u force=%d", i, detected_tags[i].size(), imagePub_[i].getNumSubscribers(), (int)force_publish_debug_);
    if (force_publish_debug_ || imagePub_[i].getNumSubscribers() > 0) {
      cv_bridge::CvImageConstPtr cv_ptr;
      try {
        cv_ptr = cv_bridge::toCvShare(msg_vec[i]);
      ...
      if (cv_ptr->image.channels() == 1) {
        ...
        RCLCPP_WARN(get_logger(), "unsupported image channels: %d", cv_ptr->image.channels());
        continue;
      }
      if (gray.rows == 0) {
        RCLCPP_ERROR(get_logger(), "empty image after conversion");
        continue;
      }
      ...
      RCLCPP_DEBUG(get_logger(), "published debug image cam %d", i);
    }
  }
}
...
void CalibrationComponent::callback2(ImageConstPtr const &img0,
  ...
  static int info_count_cb2 = 0;
  if (info_count_cb2 < 5) {
    RCLCPP_INFO(get_logger(), "callback2 sync stamps: %u.%u | %u.%u", img0->header.stamp.sec, img0->header.stamp.nanosec, img1->header.stamp.sec, img1->header.stamp.nanosec);
    info_count_cb2++;
  }
  if (single_camera_mode_) {
    processSingle(img0);
  ...
  RCLCPP_DEBUG(get_logger(), "callback2 got stamps: %u.%u and %u.%u", img0->header.stamp.sec, img0->header.stamp.nanosec, img1->header.stamp.sec, img1->header.stamp.nanosec);
