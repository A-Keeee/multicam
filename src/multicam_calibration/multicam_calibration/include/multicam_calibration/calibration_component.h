...
class CalibrationComponent : public rclcpp::Node {
 ...
 private:
  ...
  void processSingle(ImageConstPtr const &img0); // debug single camera bypass
  void subscribe();
  ...
  double approx_sync_slop_{0.3};   // seconds, configurable
  bool force_publish_debug_{false};
  bool log_raw_frames_{false};
  bool single_camera_mode_{false};
  std::vector<rclcpp::Subscription<ImageMsg>::SharedPtr> raw_log_subs_; // optional raw logging
};
}  // namespace multicam_calibration
#endif
